...
`Tests/MackastenTests/` covers `FlaggedMailContent` and `FlaggedReminderContent` (the pure builders that turn a list of items into NSMenuItem rows, with the empty / populated / wired-on-select cases) and `MenuBar.install` (the configured `NSStatusItem` reflects the input content). The AppleScript boundaries in `MailReader` and `ReminderReader` are not unit-tested — they require live `Mail.app` / `Reminders.app`. XCTest is bundled with full Xcode — CommandLineTools alone is insufficient.
...
- `Sources/Mackasten/FlaggedMailContent.swift` — pure builder turning `[MailMessage]` into `[NSMenuItem]` rows with envelope icons. Renders a "No flagged mail" placeholder when the list is empty.
...
- `Sources/Mackasten/FlaggedReminderContent.swift` — pure builder turning `[ReminderItem]` into `[NSMenuItem]` rows with circle icons. Renders a "No reminders requiring focus" placeholder when the list is empty.
